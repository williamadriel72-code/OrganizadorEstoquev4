...
public class MainActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle b) {
        super.onCreate(b);
        prefs = getSharedPreferences("bora_ifood_test_v3", MODE_PRIVATE);
        token = prefs.getString("session_token", "");
        riderId = prefs.getString("rider_id", "");
        riderName = prefs.getString("rider_name", "");
        if (token.isEmpty() || riderId.isEmpty() || riderName.isEmpty()) showLogin();
        else showOrders();
    }
    ...
    private void showLogin() {
        ...
        TextView sub = text("Escolha o seu nome. Esse mesmo nome será usado para identificar suas entregas no painel teste.", 13, "#94A3B8", false);
        sub.setGravity(Gravity.CENTER);
        LinearLayout.LayoutParams sp = full(); sp.topMargin = dp(8); sp.bottomMargin = dp(22);
        root.addView(sub, sp);

        Button choose = button("CARREGANDO MOTOBOYS...", "#27313D");
        choose.setEnabled(false);
        root.addView(choose, full());

        TextView selected = text("Nenhum nome selecionado", 13, "#CBD5E1", false);
        selected.setGravity(Gravity.CENTER);
        LinearLayout.LayoutParams selp = full(); selp.topMargin = dp(10);
        root.addView(selected, selp);
        ...
        TextView msg = text("Buscando lista de motoboys...", 12, "#94A3B8", false);
        ...
        choose.setOnClickListener(v -> {
            ...
            String[] names = riderNames.toArray(new String[0]);
            new AlertDialog.Builder(this)
                    .setTitle("Escolha seu nome")
                    .setItems(names, (d, which) -> {
                        riderId = riderIds.get(which);
                        riderName = riderNames.get(which);
                        selected.setText(riderName);
                        selected.setTextColor(Color.parseColor("#86EFAC"));
                        choose.setText("TROCAR NOME");
                        enter.setEnabled(true);
                        msg.setText("Nome selecionado. Toque em ENTRAR.");
                    })
        ...
        enter.setOnClickListener(v -> {
            if (riderId.isEmpty()) {
                msg.setText("Escolha seu nome primeiro.");
                return;
            }
            enter.setEnabled(false);
            choose.setEnabled(false);
            msg.setText("Entrando como " + riderName + "...");
            io.execute(() -> {
                try {
                    JSONObject body = new JSONObject().put("action", "login_name").put("riderId", riderId);
                    JSONObject j = post(RIDER_API, "", body, true);
                    String session = j.optString("session_token", "");
                    JSONObject rider = j.optJSONObject("rider");
                    if (session.isEmpty() || rider == null) throw new Exception("Sessão de teste não recebida.");
                    token = session;
                    riderId = rider.optString("id", riderId);
                    riderName = rider.optString("nome", riderName);
                    prefs.edit()
        ...
        setContentView(root);
        loadRiders(choose, msg);
    }

    private void loadRiders(Button choose, TextView msg) {
        io.execute(() -> {
            try {
                JSONObject j = post(RIDER_API, "", new JSONObject().put("action", "list_riders"), true);
                JSONArray a = j.optJSONArray("riders");
                riderIds.clear();
                riderNames.clear();
                if (a != null) {
                    for (int i = 0; i < a.length(); i++) {
                        ...
                        if (r == null) continue;
                        String id = r.optString("id", "").trim();
                        String nome = r.optString("nome", "").trim();
                ...
                runOnUiThread(() -> {
                    if (riderNames.isEmpty()) {
                        choose.setText("NENHUM MOTOBOY DISPONÍVEL");
                        msg.setText("Nenhum motoboy ativo foi encontrado.");
                    } else {
                        choose.setText("ESCOLHER MEU NOME");
                        choose.setEnabled(true);
                        msg.setText(riderNames.size() + " motoboy(s) disponível(is).");
                    }
                });
            } catch (Exception e) {
                runOnUiThread(() -> {
                    choose.setText("TENTAR NOVAMENTE");
                    choose.setEnabled(true);
                    msg.setText(e.getMessage() == null ? "Falha ao carregar nomes." : e.getMessage());
                    choose.setOnClickListener(v -> {
                        choose.setEnabled(false);
                        choose.setText("CARREGANDO MOTOBOYS...");
                        msg.setText("Buscando lista de motoboys...");
                        loadRiders(choose, msg);
                    });
                });
            }
    ...
    private void showOrders() {
        ...
        names.addView(text(riderName + " • iFood TESTE", 11, "#86EFAC", true));
        ...
        Button logout = button("TROCAR", "#27313D");
        LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(dp(82), dp(44)); lp.leftMargin = dp(8);
        ...
        sync = text("Sincronizando pedidos de " + riderName + "...", 12, "#94A3B8", false);
    ...
    private void refresh(boolean silent) {
        ...
        if (!silent) {
            ...
            sync.setText("Atualizando pedidos de " + riderName + "...");
        }
        io.execute(() -> {
            try {
                ...
                JSONObject rider = j.optJSONObject("rider");
                if (rider != null) {
                    String n = rider.optString("nome", "").trim();
                    if (!n.isEmpty()) riderName = n;
                }
                JSONArray a = j.optJSONArray("orders");
                ...
                runOnUiThread(() -> {
                    if (m.toLowerCase().contains("sessão expirada") || m.toLowerCase().contains("desativado")) {
    ...
    private void render(List<JSONObject> pending, List<JSONObject> done) {
        if (!onOrders || orders == null) return;
        orders.removeAllViews();
        section("NÃO CONCLUÍDOS", pending.size(), "#F8FAFC");
        if (pending.isEmpty()) {
            TextView e = text("Nenhuma entrega pendente para " + riderName + ". Quando o painel despachar um pedido para este nome, ele aparecerá aqui.", 13, "#94A3B8", false);
            e.setGravity(Gravity.CENTER);
            e.setPadding(dp(14), dp(28), dp(14), dp(28));
            orders.addView(e, full());
        } else {
            for (JSONObject o : pending) orders.addView(card(o, false), marginTop(7));
        }
        section("CONCLUÍDOS", done.size(), "#86EFAC");
        for (JSONObject o : done) orders.addView(card(o, true), marginTop(7));
        progress.setVisibility(View.GONE);
        sync.setText(riderName + " • sincronizado • atualização automática a cada 15s");
    }

    private void section(String s, int n, String c) {
        TextView t = text(s + "  •  " + n, 12, c, true);
        t.setPadding(dp(4), dp(15), dp(4), dp(7));
        orders.addView(t, full());
    }
    ...
    private View card(JSONObject o, boolean done) {
        ...
        if (!phone.isEmpty()) c.addView(text("☎ " + phone, 12, "#94A3B8", false), marginTop(4));

        if (!done) {
            Button f = button("FINALIZAR NO IFOOD", "#16A34A");
            c.addView(f, marginTop(10));
    ...
    private LinearLayout baseRoot() {
        ...
        r.setPadding(dp(24), dp(30), dp(24), dp(30));
        r.setBackgroundColor(Color.parseColor("#0B0F14"));
        return r;
    }
    ...
    private TextView text(String s, float z, String color, boolean bold) {
    ...
    private LinearLayout.LayoutParams marginTop(int t) { LinearLayout.LayoutParams p = full(); p.topMargin = dp(t); return p; }
    private int dp(int v) { return Math.round(v * getResources().getDisplayMetrics().density); }
    private String join(String a, String b) { if (a.isEmpty()) return b; if (b.isEmpty()) return a; return a + ", " + b; }
    private String joinDot(String a, String b) { if (a.isEmpty()) return b; if (b.isEmpty()) return a; return a + " • " + b; }
