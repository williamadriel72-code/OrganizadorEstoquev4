...
(function(){
 ...
 let dragging=null,resizeObs=null,lastTileSig='',mapMode='street';
 ...
 function css(){
   ...
   .g3body{display:grid;grid-template-columns:320px minmax(0,1fr);min-height:365px}.g3list{padding:10px;border-right:1px solid #ffffff10;max-height:430px;overflow:auto;background:#0c1218}.g3map{position:relative;min-height:365px;background:#d8dee2;overflow:hidden;touch-action:none;user-select:none}.g3canvas{position:absolute;inset:0;overflow:hidden;background:#d8dee2}.g3tile{position:absolute;width:256px!important;height:256px!important;max-width:none!important;max-height:none!important;object-fit:cover;user-select:none;pointer-events:none}.g3marker{position:absolute;z-index:30;transform:translate(-50%,-50%);pointer-events:none}.g3dot{width:15px;height:15px;border:3px solid #fff;border-radius:50%;box-shadow:0 2px 8px #0009}.g3marker.live .g3dot{background:#19c875}.g3marker.stale .g3dot{background:#f59e0b}.g3marker.none .g3dot{background:#64748b}.g3label{position:absolute;left:50%;bottom:20px;transform:translateX(-50%);white-space:nowrap;padding:4px 7px;border-radius:7px;background:#111c;color:#fff;font-size:10px;font-weight:900;box-shadow:0 2px 8px #0008}.g3zoom{position:absolute;z-index:50;right:10px;top:10px;display:grid;gap:5px}.g3zoom button{width:34px;height:34px;border:1px solid #0002;border-radius:8px;background:#fff;color:#111;font-size:20px;font-weight:900;box-shadow:0 2px 8px #0003}.g3layers{position:absolute;z-index:55;left:10px;bottom:24px;display:flex;gap:5px;padding:4px;border-radius:10px;background:#101820dd;box-shadow:0 2px 10px #0005}.g3layers button{border:0;border-radius:7px;padding:7px 9px;background:#2b333c;color:#dce4eb;font-size:9px;font-weight:900}.g3layers button.active{background:#fff;color:#111}.g3attr{position:absolute;z-index:40;right:4px;bottom:3px;padding:2px 5px;border-radius:4px;background:#fffd;color:#333;font-size:9px}.g3hint{position:absolute;z-index:35;left:10px;top:10px;padding:5px 8px;border-radius:8px;background:#0b1220cc;color:#dbe4ee;font-size:9px}
 ...
 function html(){
   ...
   return `<section id="bmGpsPanelV3"><div class="g3h"><div><b>⌖ GPS DOS MOTOBOYS</b><small>LOCALIZAÇÃO EM TEMPO REAL · SEM ROTAS</small></div><div><span id="g3online" style="color:#94a3b8;font-size:11px;margin-right:8px">${online} online · atualiza a cada 20s</span><button id="g3refresh" class="g3btn">↻ Atualizar GPS</button></div></div><div class="g3body"><div class="g3list">${state.error?'<div class="g3err">'+escGps(state.error)+'</div>':(rows||'<div class="g3empty">Nenhum motoboy ativo.</div>')}</div><div class="g3map" id="g3map"><div class="g3canvas" id="g3canvas"></div><div class="g3layers"><button id="g3street" class="active">Ruas HD</button><button id="g3sat">Satélite</button></div><div class="g3hint">Arraste para mover o mapa</div><div class="g3zoom"><button id="g3plus">+</button><button id="g3minus">−</button></div><div class="g3attr">© Esri · HERE · Garmin · OpenStreetMap contributors</div></div></div></section>`;
 }
 ...
 function bind(){
   ...
   const streetBtn=document.getElementById('g3street'),satBtn=document.getElementById('g3sat');
   if(streetBtn){streetBtn.classList.toggle('active',mapMode==='street');streetBtn.onclick=e=>{e.stopPropagation();mapMode='street';lastTileSig='';streetBtn.classList.add('active');satBtn?.classList.remove('active');renderMap(true)}}
   if(satBtn){satBtn.classList.toggle('active',mapMode==='sat');satBtn.onclick=e=>{e.stopPropagation();mapMode='sat';lastTileSig='';satBtn.classList.add('active');streetBtn?.classList.remove('active');renderMap(true)}}
