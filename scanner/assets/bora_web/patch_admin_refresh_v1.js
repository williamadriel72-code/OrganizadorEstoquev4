...
(function(){
 ...
 function css(){
   ...
   @media(max-width:900px){.g4h{align-items:flex-start}.g4h>div:last-child{display:flex;align-items:flex-end;flex-direction:column;gap:5px}.g4mapbar{position:sticky;top:0;z-index:60}.g4mode-status{width:100%;margin-left:0}.g4body{grid-template-columns:1fr;max-height:none}.g4list{border-right:0;border-bottom:1px solid #ffffff10;max-height:180px}.g4map{min-height:230px;height:230px}}
...
(function(){
 ...
 function installNewsButton(){
   ...
   if(!head||document.getElementById('bmNewsBtn'))return;
   const btn=document.createElement('button');btn.id='bmNewsBtn';btn.type='button';btn.className='btn secondary small';btn.textContent='Novidades';
   btn.onclick=()=>openNotice(true);
...
/* MAPAS E VALIDAÇÃO DE ENDEREÇO — diagnóstico administrativo somente leitura. */
(function bmMapDiagnosticsPanelV1(){
 if(new URLSearchParams(location.search).get('app')==='motoboy')return;
 if(window.__bmMapDiagnosticsPanelV1)return;
 window.__bmMapDiagnosticsPanelV1=true;

 const cache=new Map();
 let providerCache=null,providerLoading=false;

 function safe(v){
  try{return typeof esc==='function'?esc(String(v??'')):String(v??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]))}
  catch(_){return String(v??'')}
 }
 function pct(v){const n=Number(v);return Number.isFinite(n)?Math.round(Math.max(0,Math.min(1,n))*100)+'%':'—'}
 function statusClass(s){s=String(s||'').toUpperCase();return s==='ATIVO'?'ok':s==='NÃO CONFIGURADO'?'warn':'bad'}
 function signal(d){
  const n=String(d?.number?.status||''),c=String(d?.coordinate?.status||''),a=String(d?.gemini?.status||'');
  if(n.includes('DIVERGÊNCIA')||c.includes('DESCARTADA')||a==='blocked')return 'bad';
  if(n.includes('NÃO ENCONTRADO')||a==='warning'||a==='unavailable')return 'warn';
  return 'ok';
 }
 function signalText(k){return k==='ok'?'SEGURO':k==='warn'?'ATENÇÃO':'DIVERGÊNCIA'}

 async function diagApi(body){
  if(typeof onlineApi==='function')return onlineApi('bora-panel-map-diagnostics',body);
  const session=(await sb.auth.getSession())?.data?.session||null;
  if(!session?.access_token)throw Error('Sessão expirada.');
  const r=await fetch(U+'/functions/v1/bora-panel-map-diagnostics',{
   method:'POST',
 ...
 function installStyle(){
  if(document.getElementById('bmMapDiagnosticsStyle'))return;
  const s=document.createElement('style');s.id='bmMapDiagnosticsStyle';s.textContent=`
  .bm-map-health{margin:10px 0 12px;padding:12px;border:1px solid rgba(255,255,255,.09);border-radius:16px;background:linear-gradient(145deg,#0e151c,#0a1016)}
  .bm-map-health-head{display:flex;justify-content:space-between;gap:10px;align-items:center;margin-bottom:9px}.bm-map-health-head b{font-size:13px}.bm-map-health-head small{color:#98a2ad}
  .bm-map-provider-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(170px,1fr));gap:7px}.bm-map-provider{padding:9px;border:1px solid rgba(255,255,255,.07);border-radius:11px;background:#0b1117;min-width:0}.bm-map-provider strong{display:block;font-size:11px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.bm-map-provider small{display:block;color:#8f9aa6;font-size:9px;margin-top:4px;line-height:1.35}
  .bm-map-st{display:inline-flex;margin-top:6px;padding:3px 7px;border-radius:999px;font-size:8px;font-weight:950}.bm-map-st.ok{color:#7ef0ad;background:#22c55e1f;border:1px solid #22c55e55}.bm-map-st.warn{color:#ffd56a;background:#f59e0b1f;border:1px solid #f59e0b55}.bm-map-st.bad{color:#ff9ca8;background:#ef44441f;border:1px solid #ef444455}
  .bm-map-action-row{display:flex;justify-content:flex-end;margin-top:9px}.bm-map-open{background:#17232f!important;border:1px solid #3b82f655!important;color:#a8d0ff!important}
  .bm-map-diag{margin-top:9px;border:1px solid rgba(255,255,255,.09);border-radius:14px;background:#090e13;padding:11px}.bm-map-diag[hidden]{display:none!important}
  .bm-map-diag-head{display:flex;justify-content:space-between;gap:9px;align-items:flex-start}.bm-map-diag-head h4{margin:0;font-size:13px}.bm-map-diag-head p{margin:3px 0 0;color:#8f9aa6;font-size:9px}.bm-map-signal{padding:6px 8px;border-radius:9px;font-size:9px;font-weight:950;white-space:nowrap}.bm-map-signal.ok{color:#7ef0ad;background:#22c55e18;border:1px solid #22c55e44}.bm-map-signal.warn{color:#ffd56a;background:#f59e0b18;border:1px solid #f59e0b44}.bm-map-signal.bad{color:#ff9ca8;background:#ef444418;border:1px solid #ef444444}
  .bm-map-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin-top:9px}.bm-map-box{padding:9px;border:1px solid rgba(255,255,255,.07);border-radius:11px;background:#0e151c;min-width:0}.bm-map-box h5{margin:0 0 6px;color:#b8c3cf;font-size:9px;letter-spacing:.05em}.bm-map-box div{font-size:10px;line-height:1.45;margin:2px 0;overflow-wrap:anywhere}.bm-map-box b{color:#fff}
  .bm-map-source{padding:7px 0;border-top:1px solid rgba(255,255,255,.06);font-size:9px;line-height:1.4}.bm-map-source:first-child{border-top:0}.bm-map-source small{color:#8f9aa6}
  .bm-map-ref{margin-top:6px;padding:8px;border:1px solid rgba(255,255,255,.07);border-radius:9px;background:#090e13}.bm-map-ref b{font-size:10px}.bm-map-ref small{display:block;color:#8f9aa6;font-size:9px;margin-top:3px}
  .bm-map-frame{width:100%;height:245px;border:0;border-radius:11px;background:#10161d;margin-top:9px}.bm-map-loading{color:#8dc4ff;font-size:10px}.bm-map-error{color:#ff9ca8;font-size:10px}
  @media(max-width:720px){.bm-map-provider-grid,.bm-map-grid{grid-template-columns:1fr}.bm-map-frame{height:210px}.bm-map-health{padding:10px}}
  `;
  document.head.appendChild(s);
 }

 function healthHtml(){
  return `<div class="bm-map-health" id="bmMapHealth"><div class="bm-map-health-head"><div><b>Mapas e validação de endereço</b><small style="display:block">Integrações usadas para conferir as comandas.</small></div><button class="btn secondary small" id="bmMapHealthRefresh" type="button">Atualizar status</button></div><div id="bmMapProviderGrid" class="bm-map-provider-grid"><div class="bm-map-loading">Verificando integrações...</div></div></div>`;
 }
 ...
 async function loadProviders(force=false){
  ...
  try{
   ...
   providerCache=Array.isArray(j?.providers)?j.providers:[];
   renderProviders(providerCache);
  }catch(e){
   box.innerHTML='<div class="bm-map-error">Não foi possível consultar as integrações: '+safe(e?.message||e)+'</div>';
  }finally{providerLoading=false}
 }
 function renderProviders(rows){
  const box=document.getElementById('bmMapProviderGrid');if(!box)return;
  box.innerHTML=(rows||[]).map(p=>`<div class="bm-map-provider"><strong>${safe(p.name||p.id||'Integração')}</strong><span class="bm-map-st ${statusClass(p.status)}">${safe(p.status||'INDISPONÍVEL')}</span>${p.note?'<small>'+safe(p.note)+'</small>':''}</div>`).join('')||'<div class="bm-map-error">Nenhuma integração informada.</div>';
 }

 function mapFrame(lat,lng){
  lat=Number(lat);lng=Number(lng);if(!Number.isFinite(lat)||!Number.isFinite(lng))return '';
  const d=.006,b=[lng-d,lat-d,lng+d,lat+d].join('%2C');
  return `<iframe class="bm-map-frame" loading="lazy" referrerpolicy="no-referrer" src="https://www.openstreetmap.org/export/embed.html?bbox=${b}&layer=mapnik&marker=${lat}%2C${lng}"></iframe>`;
 }

 function renderDiagnostic(d){
  const o=d?.order||{},best=d?.maps?.best||{},ai=d?.gemini||{},sources=d?.maps?.sources||[],refs=ai?.references||[],aliases=d?.aliases||[],sig=signal(d);
  const sourceHtml=sources.length?sources.map(s=>`<div class="bm-map-source"><b>${safe(s.source||'Fonte')}</b> · ${safe(s.street||s.address||'—')}, nº <b>${safe(s.number||'não encontrado')}</b><br><small>Bairro: ${safe(s.neighborhood||'—')} · CEP: ${safe(s.zip||'—')} · Confiança: ${safe(s.confidence||'—')}${s.approximate?' · APROXIMADO':''}</small></div>`).join(''):'<div class="bm-map-source">Nenhum resultado retornado.</div>';
  const refHtml=refs.length?refs.map(r=>`<div class="bm-map-ref"><b>${safe(r.name||r.text||'Referência')}</b><small>${safe(r.relation||'')} · ${safe(r.type||'')} · confiança ${pct(r.confidence)}</small><div>${safe(r.text||'')}</div>${r.map_match?'<small>Localizada por '+safe(r.map_match.source||'mapa')+': '+safe(r.map_match.address||r.map_match.street||'—')+(r.map_match.distance_to_delivery_m!=null?' · '+safe(r.map_match.distance_to_delivery_m)+' m da entrega':'')+'</small>':'<small>'+(r.searchable?'Referência não localizada automaticamente nos mapas.':'Característica visual — preservada para conferência.')+'</small>'}</div>`).join(''):'<div class="bm-map-source">Nenhuma referência identificada pela IA.</div>';
  const aliasHtml=aliases.length?aliases.map(a=>safe((a.old_name||'')+' → '+(a.current_name||''))).join('<br>'):(ai.old_street_name||ai.current_street_name?safe((ai.old_street_name||o.street||'')+' → '+(ai.current_street_name||ai.canonical_street||'')):'—');
  return `<div class="bm-map-diag-head"><div><h4>Diagnóstico da comanda #${safe(o.display_id||String(o.order_id||'').slice(0,8))}</h4><p>Somente leitura · o número original não é substituído.</p></div><span class="bm-map-signal ${sig}">${signalText(sig)}</span></div>
   <div class="bm-map-grid">
    <div class="bm-map-box"><h5>ENDEREÇO DA COMANDA</h5><div><b>Rua:</b> ${safe(o.street||'—')}</div><div><b>Número original:</b> ${safe(o.number_original||'—')}</div><div><b>Bairro informado:</b> ${safe(o.neighborhood_original||'—')}</div><div><b>CEP:</b> ${safe(o.zip||'—')}</div><div><b>Referência:</b> ${safe(o.reference_original||'—')}</div></div>
    <div class="bm-map-box"><h5>RESULTADO PRINCIPAL DOS MAPAS</h5><div><b>Fonte:</b> ${safe(best.source||'—')}</div><div><b>Rua:</b> ${safe(best.street||best.address||'—')}</div><div><b>Número encontrado:</b> ${safe(best.number||'não encontrado')}</div><div><b>Bairro real:</b> ${safe(best.neighborhood||ai.canonical_neighborhood||'—')}</div><div><b>CEP:</b> ${safe(best.zip||ai.canonical_zip||'—')}</div><div><b>Lat/Lng:</b> ${safe(best.lat??'—')} / ${safe(best.lng??'—')}</div></div>
    <div class="bm-map-box"><h5>VALIDAÇÃO</h5><div><b>Número:</b> ${safe(d?.number?.status||'—')}</div><div><b>Coordenada:</b> ${safe(d?.coordinate?.status||'—')}</div><div>${safe(d?.coordinate?.reason||'')}</div><div><b>Confiança do mapa:</b> ${safe(best.confidence||'—')}</div><div><b>Exata/aproximada:</b> ${best.approximate?'APROXIMADA':'EXATA / MELHOR RESULTADO'}</div></div>
    <div class="bm-map-box"><h5>RESULTADO DA IA — GEMINI</h5><div><b>Status:</b> ${safe(ai.status||'—')}</div><div><b>Confiança:</b> ${pct(ai.confidence)}</div><div><b>Rua validada:</b> ${safe(ai.canonical_street||o.street||'—')}</div><div><b>Bairro real:</b> ${safe(ai.canonical_neighborhood||o.neighborhood_original||'—')}</div><div><b>CEP:</b> ${safe(ai.canonical_zip||o.zip||'—')}</div><div><b>Motivo:</b> ${safe(ai.reason||'Sem conclusão da IA.')}</div></div>
    <div class="bm-map-box"><h5>NOME ANTIGO / ATUAL DA RUA</h5><div>${aliasHtml}</div></div>
    <div class="bm-map-box"><h5>REFERÊNCIAS IDENTIFICADAS PELA IA</h5>${refHtml}</div>
    <div class="bm-map-box" style="grid-column:1/-1"><h5>RESULTADO SEPARADO POR FONTE</h5>${sourceHtml}</div>
   </div>
   ${mapFrame(best.lat,best.lng)}`;
 }

 async function openDiag(id,box,btn){
  if(!box)return;
  if(!box.hidden){box.hidden=true;if(btn)btn.textContent='Mapas e validação';return}
  box.hidden=false;if(btn)btn.textContent='Fechar diagnóstico';
  if(cache.has(id)){box.innerHTML=renderDiagnostic(cache.get(id));return}
  box.innerHTML='<div class="bm-map-loading">Consultando mapas e analisando referências com a IA...</div>';
  try{
   const j=await diagApi({action:'diagnose_order',orderId:id});
   cache.set(id,j);box.innerHTML=renderDiagnostic(j);
  }catch(e){box.innerHTML='<div class="bm-map-error">Falha no diagnóstico: '+safe(e?.message||e)+'</div>'}
 }
 ...
 if(typeof onlineOrdersArea==='function'){
  const prevArea=onlineOrdersArea;
  onlineOrdersArea=function(){
   const html=prevArea();
   return html.replace('<div class="online-tabs">',healthHtml()+'<div class="online-tabs">');
  };
 }

 if(typeof onlineOrderCard==='function'){
  const prevCard=onlineOrderCard;
  onlineOrderCard=function(o){
   let html=prevCard(o);
   const id=safe(o?.order_id||'');
   const extra=`<div class="bm-map-action-row"><button type="button" class="btn secondary small bm-map-open" data-bm-map-diag="${id}">Mapas e validação</button></div><div class="bm-map-diag" id="bmMapDiag-${id}" hidden></div>`;
   return html.replace('</article>',extra+'</article>');
  };
 }

 if(typeof renderOnlineOrders==='function'){
  const prevRender=renderOnlineOrders;
  renderOnlineOrders=function(){const r=prevRender();setTimeout(bind,0);return r};
 }

 if(typeof bindOnlineOrders==='function'){
  const prevBind=bindOnlineOrders;
