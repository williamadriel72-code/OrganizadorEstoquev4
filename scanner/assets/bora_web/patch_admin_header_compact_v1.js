...
function bmReportPersonHtml(m){
 const r=m.resumo||{},hasArrival=!!m.chegada_at,arrival=hasArrival?bmReportTime(m.chegada_at):'Não registrada',diaria=hasArrival?Number(m.diaria||0):0,total=hasArrival?Number(r.total_dia||0):Number(r.taxas||0),comandas=Number(r.entregas||0),comandaLabel=comandas===1?'comanda':'comandas';
 return `<details class="bm-report-person"><summary><span>${comandas} ${comandaLabel} · ${esc(m.nome||'Motoboy')}</span><span>${BRL(total)}</span></summary><div class="bm-report-person-body"><div class="bm-report-mini"><span>Chegada: <b>${esc(arrival)}</b></span><span>Diária: <b>${BRL(diaria)}</b></span><span>Entregas: <b>${Number(r.entregas||0)}</b></span><span>Confirmadas: <b>${Number(r.confirmadas||0)}</b></span><span>Saídas: <b>${Number(r.saidas||0)}</b></span><span>Taxas: <b>${BRL(r.taxas||0)}</b></span>${Number(r.canceladas||0)?`<span>Canceladas: <b>${Number(r.canceladas||0)}</b></span>`:''}</div><div class="bm-report-section-title">Notas do dia</div>${bmReportDeliveryRows(m.entregas)}<div class="bm-report-section-title">Saídas do dia</div>${bmReportOutingRows(m.saidas)}</div></details>`;
}
