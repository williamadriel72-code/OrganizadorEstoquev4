...
function bmRiderNoteCard(e){
 ...
 const cardStyle=confirmed
  ? 'padding:16px;margin:10px 0;border:2px solid #21c875;border-radius:14px;background:rgba(33,200,117,.22);box-shadow:0 0 0 1px rgba(33,200,117,.12) inset'
  : 'padding:16px 0;border-bottom:1px solid rgba(255,255,255,.08)';
 return `<div data-note-card="${e.id}" style="${cardStyle}">
  <div class="stat-label" style="${confirmed?'color:#66f0ae':''}">NÚMERO DA NOTA</div>
  <div data-note-number style="font-size:34px;font-weight:900;letter-spacing:1px;margin:4px 0 8px;${confirmed?'color:#66f0ae':''}">${esc(e.nota_numero)}</div>
  ${confirmed
   ? `<div style="margin-top:8px;padding:13px 14px;border-radius:10px;background:#159958;color:#fff;font-weight:900;text-align:center">✓ NOTA CONFIRMADA${confirmedAt?` · ${confirmedAt}`:''}</div>`
   : `<div class="row-sub" style="margin-bottom:10px">Compare este número com a nota que você pegou. Se estiver igual, confirme abaixo.</div><button type="button" class="btn" style="width:100%;font-weight:900;background:#159958!important;color:#fff!important;border:1px solid #21c875!important;min-height:48px" onclick="bmConfirmRiderNote('${e.id}')" data-confirm-note="${e.id}">CONFIRMAR NOTA</button>`}
 </div>`;
}
...
todayHtml=function(d){
 ...
 return `<section class="rider-fast-panel">
  ...
  <div class="notice"><b>${pending?`${pending} nota${pending===1?'':'s'} aguardando confirmação`:'Todas as notas estão confirmadas'}</b><br>Confira apenas se o número exibido é o mesmo da nota física.</div>
...
async function bmConfirmRiderNote(id){
 ...
 const oldBtnText=btn?.textContent||'CONFIRMAR NOTA';
 if(btn){
  btn.disabled=true;
  btn.textContent='CONFIRMANDO...';
  btn.style.background='#159958';
  btn.style.color='#fff';
 }
 if(card){
  card.style.transition='background .15s,border .15s';
  card.style.border='2px solid #21c875';
  card.style.borderRadius='14px';
  card.style.padding='16px';
  card.style.margin='10px 0';
  card.style.background='rgba(33,200,117,.14)';
 }
 try{
  ...
  if(card){
   card.style.background='rgba(33,200,117,.24)';
   const noteNumber=card.querySelector('[data-note-number]');
   if(noteNumber)noteNumber.style.color='#66f0ae';
   if(btn)btn.outerHTML=`<div style="margin-top:8px;padding:13px 14px;border-radius:10px;background:#159958;color:#fff;font-weight:900;text-align:center">✓ NOTA CONFIRMADA</div>`;
  }
  ...
  setTimeout(async()=>{try{await renderRider('Hoje')}catch(_){ }finally{if(rider.silentConfirmId===id)rider.silentConfirmId=null}},500);
 }catch(e){
  ...
  if(btn){btn.disabled=false;btn.textContent=oldBtnText;btn.style.background='#159958';btn.style.color='#fff'}
